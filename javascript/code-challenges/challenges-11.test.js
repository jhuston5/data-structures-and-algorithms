...
function transformToLis(obj) {
  ...
  // Map through an object - or maybe two forEach?
  // Obj.keys
...
const count = (target, input) => {
  // Solution code here...
  // could use filter to identify 5
  // could use reduce to take out everything that does not include 5 then return that element
  ...
  // filter each array
  // return the length of each array that has been filtered, added together
  let counter = 0;
  input.map((arr) => {
    let newArr = arr.filter(element => target === element).length;
    counter += newArr;
  });
  return counter;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  // Access the array within the arrays - map?
  // reduce that array given those parameters
  // raise those arrays 2 to the power of the resulting number (probably a forEach)
  // return an array of arrays
  // input.map((arr) => {
  //   arr.map(element => {
  //     if (isNaN(element) && (element % 5 === 1)) {
  //       newArr.push(element);
  //     }
  //   });
  // });
...
let findMaleAndFemale = (data) => {
  ...
  // let starWarsArr = [];
  // data.map((key) => {
  //   if (Object.key.gender === 'female' || Object.key.gender === 'male') {
