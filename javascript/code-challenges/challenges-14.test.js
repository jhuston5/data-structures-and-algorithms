...
const toTitleCase = (arr) => {
  ...
  let newArr = arr.map((str) => {
    ...
    let lowerStr = str.substr(1);
    let fullStr = upperChar + lowerStr;
    return fullStr;
  });
  return newArr;
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  // foreach through the star wars array
  // set a check varaibel for luke's mass
  // check every character's mass against luke's
  // set a charString variable
  // if the mass is larger, add to the charString variable
  let charString = '';
  let lukeMass = 77;
  arr.forEach(character => {
    if (parseInt(character.mass) > lukeMass) {
      charString = `${charString} - ${character.name}`;
    }
  });
  return charString.substr(3);
};
...
const sortBy = (property, arr) => {
  ...
  // sort array by price
  // console.log(arr);
  // console.log( arr.sort(arr.property));
  return arr.sort(arr.property);
};
